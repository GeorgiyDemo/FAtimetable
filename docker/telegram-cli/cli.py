...
class TelegramCli(object):
    ...
    def handler(self):
        ...
        for update in self.bot.get_updates(offset=self.update_id, timeout=10):
            ...
            if update.message.text == "/start":
            ...
            elif update.message.text.split(" ")[0] == "/stats" and self.check_admin(tg_userid) == True:
                ...
                if len(args_list) != 3:
                    ...
                    update.message.reply_text(
                        ...
                        parse_mode=p_mode)
                    r = requests.post("http://" + IP_ADDR + "/add_number",
                                      data={"number": phone_number, "group": group}).json()
                ...
                if r["status"] == "ok":
                    self.bot.send_message(chat_id=tg_userid,
                                          text="Успешная перезапись пользователя " + input_number + " в группу " + input_group,
                                          reply_markup=reply_markup)
                else:
                    self.bot.send_message(chat_id=tg_userid,
                                          text="Что-то совсем всё плохо\nПиши @Georgiy_D и рассказывай как ты дошёл до такой жизни",
                                          reply_markup=reply_markup)
